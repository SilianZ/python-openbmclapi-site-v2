...
2. 根据[配置文件](/docs/advanced/configuration)编辑 `/opt/py-openbmclapi/config/config.yml`。

3. 重新启动程序：

    ```sh
    systemctl reload py-openbmclapi.service
    ```

**相关命令：**
`systemctl start py-openbmclapi.service` - 启动 python-openbmclapi 服务。
`systemctl stop py-openbmclapi.service` - 关闭 python-openbmclapi 服务。
`systemctl reload py-openbmclapi.service` - 重新启动 python-openbmclapi 服务以应用最新配置。
`journalctl -f --output cat -u py-openbmclapi.service` - 实时查看 python-openbmclapi 的日志。
